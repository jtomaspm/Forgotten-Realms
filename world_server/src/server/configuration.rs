...
pub async fn get_configuration(world: Option<String>) -> WorldServerConfig {
    ...
    if fs::try_exists(format!("{}/config.json", config_path)).await.unwrap_or(false) {
        ...
        let config: WorldServerConfig = serde_json::from_str(&config_content).expect("Could not parse configuration file");
        return config;
    } else {
        let default_config = WorldServerConfig::default();
